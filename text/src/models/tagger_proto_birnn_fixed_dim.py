...
class TaggerProtoBiRNNFixed(TaggerBase):
    ...
    def __init__(self, word_seq_indexer, tag_seq_indexer, class_num, batch_size=1, rnn_hidden_dim=100,
                 ...
                 similarity_function_name = 'gaussian'):
        super(TaggerProtoBiRNNFixed, self).__init__(word_seq_indexer, tag_seq_indexer, gpu, batch_size)
    ...
    def forward(self, word_sequences):
        ...
        proto_output_h, distances = self.proto_layer(pooled_output_h) # proto_output shape: batch_size x num_features
    ...
    def push_forward(self, word_sequences):
        ...
        pooled_output_h = self.pooler(rnn_output_h, mask) # shape: batch_size x hidden_dim*2
        proto_output_h, distances = self.proto_layer(pooled_output_h) # proto_output shape: batch_size x num_features
        return pooled_output_h, distances # latent shape: batch_size x proto_dim. distances shape: batch_size x num_prototypes
    ...
    def freeze_unfreeze_parameters(self, epoch, args):
        ...
        assert args.unfreeze_lin_layer > 999, "Layers should be kept frozen in this model"
        assert args.unfreeze_feature_extractor > 999, "Layers should be kept frozen in this model"
    ...
    def give_similar_case(self, word_sequence):
        ...
        # compose the explanation str
        explanation = "Similar case (label: %s): \n" % activated_prototype.tag + \
