...
def find_k_nearest_latents_to_prototypes(args, tagger, word_sequences, tag_sequences, data_name, k = 5):
    ...
    for n in range(n_batches):
        ...
        mask = tagger.get_mask_from_word_sequences(batch)            
    ...
    with open(fname, 'w') as f:
            ...
            for m in range(n_protos):
                ...
                saliency_str = tagger.prototype_saliency_map(prototype.context,
                                                                    saliency_type = args.saliency_type,
                                                                    prototype_id = m)
                f.write('%s \n----\n %s \n\n' % (prototype.to_str(), saliency_str))
                for i in range(k):
                    latent = heaps[m][i]
                    global_idx = latent.global_idx
                    f.write('Nearest neighbor %d, class: %d, tag: %s, global_idx: (%d,%d) \n' % (i, latent.class_id, latent.tag,
                                                                                                global_idx[0], global_idx[1]))
                    saliency_str = tagger.prototype_saliency_map(latent.context,
                                                                    saliency_type = args.saliency_type, 
                                                                    prototype_id = m)
                    f.write('%s \n----\n %s \n\n' % (latent.to_str(), saliency_str))
...
if __name__ == '__main__':
    ...
    find_k_nearest_latents_to_prototypes(args=args,
                                         tagger=tagger,
                                         word_sequences=word_sequences_train,
                                         tag_sequences=tag_sequences_train,
                                         data_name="train",
                                         k=5)
