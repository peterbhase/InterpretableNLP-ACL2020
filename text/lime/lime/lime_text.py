...
class LimeTextExplainer(object):
    ...
    def __data_labels_distances(self,
        ...
        # remove examples that are empty strings
        empty_strings = [len(text.split()) == 0 for text in inverse_data]
        inverse_data = [inverse_datum for idx, inverse_datum in enumerate(inverse_data) if not empty_strings[idx]]
        data = np.array([datum for idx, datum in enumerate(data) if not empty_strings[idx]])
        labels = classifier_fn(inverse_data)
        distances = distance_fn(sp.sparse.csr_matrix(data))
        return data, labels, distances


    def safe_explain_instance(self, text, classifier_fn, num_attempts = 20, num_features = 5):
        explanation_found = False
        for i in range(num_attempts):
            try:
                lime_explanation = self.explain_instance(
                    text_instance=text,
                    classifier_fn = classifier_fn,
                    num_features = num_features
                    )     
                explanation = lime_explanation.to_str()
        ...
        if not explanation_found:
            print("Failed to get LIME for text: \n %s" % text)
