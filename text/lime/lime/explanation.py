...
class Explanation(object):
    ...
    def to_str(self):
        feature_weights = self.as_list()
        # explanation = 'Explanation for input: %s \n' %s
        # explanation += "Classes: {%s, %s}\n" % (self.class_names[0], self.class_names[1])
        explanation = ''

        # flip weights if 'pos' is the 0 class. but flip weights carefully -- use -weight and 1-intercept
        
        if self.class_names[0] == 'pos':
            feature_weights.sort(key = lambda tup : tup[1], reverse = False)
            pos_weight_sum = 0
            neg_weight_sum = 0
            ...
            explanation += "Baseline evidence: %.2f \n" % (1-self.intercept[1])
            explanation += "Sum of weights: %.2f \n" % (pos_weight_sum + neg_weight_sum)
            explanation += "Total evidence: %.2f" % (1-self.intercept[1] + pos_weight_sum + neg_weight_sum)
        elif self.class_names[0] == 'neg':
            feature_weights.sort(key = lambda tup : tup[1], reverse = True)
            weight_sum = 0
            explanation += "Positive numbers are evidence toward the instance being positive sentiment\n"
            explanation += "Baseline evidence (model intercept): %.1f \n" % self.intercept[1]
            explanation += 'Features: \n'
            for word, weight in feature_weights:
                explanation += "Word: %12s   |   weight: %4.2f \n" % (word,weight)
                weight_sum += weight if weight > 0 else 0
            explanation += "Sum of positive weights: %.3f (added when every positive word is present)\n" % weight_sum            
        else:
            pos_class = self.class_names[1]
            feature_weights.sort(key = lambda tup : tup[1], reverse = True)
            pos_weight_sum = 0
            neg_weight_sum = 0
            explanation += "Positive numbers are evidence toward the instance being class %s \n" % pos_class
            explanation += "Baseline evidence (model intercept): %.1f \n" % self.intercept[1]
            explanation += 'Features: \n'
            for word, weight in feature_weights:
                explanation += "Word: %12s   |   weight: %4.2f \n" % (word,weight)
                pos_weight_sum += -weight if -weight > 0 else 0
                neg_weight_sum += -weight if -weight < 0 else 0
            explanation += "Sum of positive weights: %.3f (added when every positive-weighted word is present)\n" % pos_weight_sum
            explanation += "Sum of negative weights: %.3f (added when every with with a negative weight is present)" % neg_weight_sum
