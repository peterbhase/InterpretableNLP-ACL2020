...
class AnchorExplanation:
    ...
    def __init__(self, type_, exp_map, as_html, confidence = .95):
        self.type = type_
...
and only_same_prediction at the same time')
    ...
    def __str__(self):
        explanation = self.to_str()
        return explanation

    def to_str(self):
        classes = ['pos','neg']
        predicted_tag = classes[self.exp_map['prediction']]
        explanation = "Conditions for model predicting \'%s\':\n" % predicted_tag
        for i, word in enumerate(self.names(),start=1):
            explanation += "%d. %s \n" % (i,word)
        return explanation
