...
class AnchorText(object):
    ...
    def get_sample_fn(self, text, classifier_fn, use_proba=False):
        ...
        def sample_fn(present, num_samples, compute_labels=True):
            if self.use_unk_distribution:
                ...
                raw_data, data, orig_seq = utils.perturb_sentence(
                    text, present, num_samples, self.neighbors, 
                    top_n=100, # 100
                    proba_change = .5, #.5
                    temperature = .4, #.4
                    use_proba=use_proba)
            labels = []
            if compute_labels:
                labels = 1 * (classifier_fn(raw_data) == np.array(true_label))
    ...
    def explain_instance(self, text, classifier_fn, threshold=0.95,
        ...
        exp = anchor_base.AnchorBaseBeam.anchor_beam(
            ...
            desired_confidence=threshold, stop_on_first=True, verbose = verbose, **kwargs)
        ...
        if verbose:
            ...
            print("Anchor coverage: %.2f" % explanation.coverage())
        return explanation

    def safe_explain_instance(self, text, classifier_fn, threshold, num_attempts = 10, verbose = False, short_exp = True):
        '''
        wrapper for explain_instance. tries num_attempt times to find an explanation. returns the explanation.to_str()
        for each failure, increment the tolerance parameters on explain_instance
        '''
        anchor_found = False
        num_tries = 0
        delta = .1 # this is delta from the paper
        tau= .15 # NOT the tau from the paper. the lower bound on MAB precision estimates is threshold - tau
        for i in range(num_attempts):
            try:        
                ...
                if len(explanation.names()) > 0:
                    anchor_found = True
                    explanation = explanation.to_str_short() if short_exp else explanation.to_str()
                    # print("Num tries to find an anchor: %d" % num_tries)
                    break
                else:                    
                    num_tries += 1
                    # increment these tolerance parameters
                    # delta += 1e-4
                    # tau += 1e-4
                    print("num tries for Anchor: %d" % num_tries, end = '\r')
            except:
                num_tries += 1
                # increment these tolerance parameters
                # delta += 1e-4
        ...
        if not anchor_found:
            print("Failed to get anchor for instance: \n %s" % text)
