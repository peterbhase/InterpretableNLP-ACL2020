...
class Report():
    def __init__(self, fn, args, score_names):
        ...
        if os.path.isfile(fn) and args.load_name == args.save_name:
            print("Loading report from %s" % fn)
            self.text = open(fn,'r').read()
            self.text += "\n\nCONTINUING REPORT\n\n"
        else:
            print("Starting new report at %s" % fn)
    ...
    def write_final_line_score(self, final_score):
        self.text += '\n\n Best performance: %1.4f' % final_score
        self.__save()
