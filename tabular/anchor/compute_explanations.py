...
def main():
    ...
    parser.add_argument('-d', dest='dataset', default = 'adult', 
                        choices=['adult', 'recidivism', 'lending'],
                        help='dataset to use')
    parser.add_argument('-e', dest='explainer', default = 'lime',
                        choices=['lime', 'anchor'],
                        help='explainer, either anchor or lime')
    parser.add_argument('-m', dest='model', default = 'xgboost', 
    ...
    if args.model == 'logistic':
        c = sklearn.linear_model.LogisticRegression(solver = 'lbfgs')
        c.fit(explainer.encoder.transform(dataset.train), dataset.labels_train)
    if args.model == 'nn':
