...
class AnchorTabularExplainer(object):
    ...
    def __init__(self, class_names, feature_names, data=None,
        ...
        self.feature_to_order = {name : idx for idx, name in enumerate(self.feature_names)}
    ...
    def sample_from_train(self, conditions_eq, conditions_neq, conditions_geq,
        ...
        idx = np.random.choice(range(train.shape[0]), int(num_samples),
                               replace=True)
        sample = train[idx]
    ...
    def explain_lime(self, data_row, predict_proba_fn, num_features=5,
                     with_intercept=True, use_same_dist=False, encode_before_forward = False):
        """predict_proba_fn is original function"""
        def predict_fn(x):
            if encode_before_forward:
    ...
    def explain_lime_str(self, data_row, predict_proba_fn, num_features=5,
                     with_intercept=True, use_same_dist=False, encode_before_forward = False):
        '''
        wrapper for explain_lime that nicely formats the model output
        '''

        assert self.class_names[1] == 'above $50K', "Wrote explain_lime_str assuming class 1 is \"above $50K\""

        explanation = self.explain_lime(data_row, predict_proba_fn, num_features=num_features,
                     with_intercept=with_intercept, use_same_dist=use_same_dist, encode_before_forward = encode_before_forward)

        # get feature weights
        feature_weights = explanation['linear_model']
        ...
        pos_weight_sum = 0
        neg_weight_sum = 0
        explanation_str += 'Features: \n'
        for word, weight in feature_weights:
            explanation_str += "%s   |  %3.2f \n" % (word,weight)
            pos_weight_sum += weight if weight > 0 else 0
            neg_weight_sum += weight if weight < 0 else 0
        explanation_str += "Baseline: %.2f \n" % (explanation['intercept'])
        # explanation_str += "Sum of positive weights: %2.3f \n" % pos_weight_sum
        # explanation_str += "Sum of negative weights: %2.3f " % neg_weight_sum
        # explanation_str += "\n------\n"
        explanation_str += "Sum of weights: %2.2f \n" % (pos_weight_sum + neg_weight_sum)
        # explanation_str += "\n------\n"
        explanation_str += "Total evidence: %2.2f " % (explanation['intercept'] + pos_weight_sum + neg_weight_sum)
    ...
    def get_sample_fn(self, data_row, classifier_fn,
        ...
        def sample_fn(present, num_samples, compute_labels=True):
            ...
            data = np.zeros((int(num_samples), len(mapping)), int)
            for i in mapping:
                f, op, v = mapping[i]
                if op == 'eq':
            ...
            labels = []
            if compute_labels:
                labels = 1 * (predict_fn(raw_data) == true_label)
    ...
    def explain_anchor_str(self, data_row, classifier_fn, threshold=1,
                      delta=0.05, tau=0.1, batch_size=10,
                      max_anchor_size=None,
                      desired_label=None,
                      sample_whole_instances=True, **kwargs):
        # this used to be a longer print version, relative to the explain_anchor_str_short

        # get anchor explanation
        explanation = self.explain_lucb_beam(data_row, classifier_fn, threshold=threshold,
                          delta=delta, tau=tau, batch_size=batch_size,
                          max_anchor_size=max_anchor_size,
                          desired_label=desired_label,
                          sample_whole_instances=sample_whole_instances,
                          verbose = False)
        
        # get model prediction on instance
        model_prediction = self.class_names[int(classifier_fn(data_row))]

        def get_var_from_rule(rule):
            # obviously hardcoded to census data
            if 'Hours' or 'Age' in rule:
                return rule.split()[0]
            else:
                return rule.split('=')[0].strip()

        # get explanation as str
        rule_list = explanation['names']
        rule_list = sorted(rule_list, key = lambda x: self.feature_to_order[get_var_from_rule(x)])
        explanation_str = "Conditions for the model predicting \'%s\': \n" % (model_prediction)
        for i, word in enumerate(rule_list,start=1):
            explanation_str += "%d. %s \n" % (i, word)

        return explanation_str

    def explain_anchor_str_short(self, data_row, classifier_fn, threshold=1,
                      delta=0.05, tau=0.1, batch_size=10,
                      max_anchor_size=None,
                      desired_label=None,
                      sample_whole_instances=True, **kwargs):
        ...
        explanation_str = "Conditions for the model predicting \'%s\': \n" % (model_prediction)
        for i, word in enumerate(rule_list,start=1):
            explanation_str += "%d. %s \n" % (i, word) if i != len(explanation['names']) else "%d. %s" % (i,word)

        return explanation_str

    def explain_anchor_str_inv(self, data_row, classifier_fn, threshold=1,
                      delta=0.05, tau=0.1, batch_size=10,
                      max_anchor_size=None,
                      desired_label=None,
                      sample_whole_instances=True, **kwargs):
        ...
        # get model prediction on instance
        model_pred = int(classifier_fn(data_row))
        opposite_pred = 1-model_pred
        predicted_tag = self.class_names[model_pred]
        opposite_tag = self.class_names[1-model_pred]

        # get anchor explanation
        explanation = self.explain_lucb_beam(data_row, classifier_fn, threshold=threshold,
                          delta=delta, tau=tau, batch_size=batch_size,
                          max_anchor_size=max_anchor_size,
                          desired_label=opposite_pred, # find conditions that would change the prediction
                          sample_whole_instances=sample_whole_instances,
                          verbose = False)

        # get explanation as str
        explanation_str = "Evidence in favor of \'%s\': \n" % opposite_tag
        for i, word in enumerate(explanation['names'],start=1):
            explanation_str += "%d. %s \n" % (i, word)

        print(explanation['max_precision'])
        print(explanation['last_coverage'])
